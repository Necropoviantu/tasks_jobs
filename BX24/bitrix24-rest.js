(function(){
    ...
    */
    ...
    BX24.initAuth = function(config) {
        PARAMS.CLIENT_ID = config.client_id;
        PARAMS.CLIENT_SECRET = config.client_secret;
        PARAMS.DOMAIN = config.domain;
        PARAMS.LANG = config.lang;
        PARAMS.AUTH_ID = config.access_token;
        PARAMS.REFRESH_ID = config.refresh_token;
        PARAMS.MEMBER_ID = config.member_id;
        PARAMS.AUTH_EXPIRES = config.expires_in;
        PARAMS.PLACEMENT = config.placement || "DEFAULT";
        PARAMS.PLACEMENT_OPTIONS = config.placement_options || {};
        
        if(PARAMS.AUTH_EXPIRES < (new Date()).valueOf()) BX24.refreshAuth(() => {
            util.ready();
            util.init();
        });
        else {
    ...
    */
    ...
    BX24.refreshAuth = function(cb)
    {
        let xhr = new XMLHttpRequest,
            url = "https://" + PARAMS.DOMAIN + "/oauth/token/?grant_type=refresh_token&refresh_token=" + PARAMS.REFRESH_ID + "&client_id=" + PARAMS.CLIENT_ID + "&client_secret=" + PARAMS.CLIENT_SECRET;
        xhr.open("POST", url);
		...
		{
			xhr.onload = function() {};
			let bSuccess = ajax.isSuccess(xhr);
			if(bSuccess)
			{
                try {
                    let data = JSON.parse(xhr.responseText);
                    PARAMS.AUTH_ID = data.access_token;
                    PARAMS.MEMBER_ID = data.member_id;
                    PARAMS.REFRESH_ID = data.refresh_token;
                    PARAMS.DOMAIN = data.domain;
                    PARAMS.AUTH_EXPIRES = data.expires * 1000;
    ...
    if(typeof exports != "undefined") module.exports = window.BX24;
})();